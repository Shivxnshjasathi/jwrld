...
import { subscribeToDirectMessages, sendDirectMessage } from '@/lib/firestore';
...
export default function FriendChatPage() {
  ...
  useEffect(() => {
    ...
    const fetchFriend = async () => {
      ...
      if (snap.exists()) {
        setFriendName(snap.data().name || 'Friend');
      }
    };
    fetchFriend();
  }, [friendId]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToDirectMessages(user.uid, friendId, (msgs) => {
      setMessages(msgs);
    });
        ...
        <div className="flex items-center gap-md">
          ...
          <div className="flex items-center gap-sm">
            ...
            <div className="flex flex-col">
              <span className="font-headline-md text-[20px] font-bold text-on-surface tracking-tight leading-none">{friendName}</span>
        ...
        <div className="flex-1 overflow-y-auto chat-scroll px-gutter py-lg flex flex-col gap-lg z-10" ref={scrollRef}>
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
              ...
              <h3 className="text-white font-bold mb-1 text-[20px]">Say hi to {friendName}!</h3>
              <p className="text-sm text-on-surface-variant">Send a message to start chatting.</p>
            </div>
          ) : (
            <>
              <div className="flex justify-center">
                <span className="font-label-sm text-[12px] font-bold text-on-surface-variant/60 glass-panel px-md py-xs rounded-full">Chat</span>
              </div>
